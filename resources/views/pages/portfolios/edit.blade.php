        ...
        <form action="{{route('admin.portfolios.update', $portfolio->id)}}" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="form-group col-md-4 mt-3">
                    <h3>Image</h3>
                    <img style="height: 30vh" src="{{Storage::url($portfolio->gambar)}}" class="img-thumbnail">
                ...
                <div class="form-group col-md-4 mt-3">
                    <div class="mb-5">
                        ...
                        <input type="text" class="form-control @error('nama_app') is-invalid @enderror" id="nama_app" name="nama_app" value="{{ old('nama_app') ? old('nama_app') : $portfolio->nama_app}}"/>
                        @error('nama_app') <div class="text-muted">{{ $message }}</div> @enderror
                    </div>
                    <div class="mb-5">
                        <label for="desc"><h4>Description</h4></label>
                        <textarea name="desc"
                              class="ckeditor form-control @error('desc') is-invalid @enderror">{{ old('desc') ? old('desc') : $portfolio->desc}}</textarea>
                        @error('desc') <div class="text-muted">{{ $message }}</div> @enderror
                    </div>
                    <div class="mb-5">
                        <label for="github"><h4>Github</h4></label>
                        <input type="text" class="form-control @error('github') is-invalid @enderror" id="github" name="github" value="{{ old('github') ? old('github') : $portfolio->github}}"/>
